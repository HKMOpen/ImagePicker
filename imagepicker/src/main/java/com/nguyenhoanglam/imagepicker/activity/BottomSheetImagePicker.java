...
import static com.nguyenhoanglam.imagepicker.helper.Constants.INTENT_EXTRA_LAYOUT_RES;
...
import static com.nguyenhoanglam.imagepicker.helper.Constants.INTENT_EXTRA_SHEETPEEK_HEIGHT;
...
public class BottomSheetImagePicker extends BottomSheetDialogFragment implements OnImageClickListener {

    private static final String TAG = "ImageFragmentPicker";
    public final static String info_tag = "select.situation.image";
    private Spinner mSpinner;
    public final static int LAYOUT_RES = R.layout.simple_bottom_sheet;

    private BottomSheetBehavior mBottomSheetBehavior;
    private List<Folder> listfolderstr;
    private ImageFolderDetection mDetection;
    ...
    public static BottomSheetImagePicker newInstance(Bundle in) {
        BottomSheetImagePicker bottomSheetDialogFragment = new BottomSheetImagePicker();
        bottomSheetDialogFragment.setArguments(in);
    ...
    private BottomSheetBehavior.BottomSheetCallback mBottomSheetBehaviorCallback = new BottomSheetBehavior.BottomSheetCallback() {
        ...
        public void onStateChanged(@NonNull View bottomSheet, int newState) {
            ...
            /* if (newState == BottomSheetBehavior.STATE_EXPANDED) {
                mBottomSheetBehavior.setPeekHeight(getResources().getDimensionPixelOffset(R.dimen.filter_start));
            }
            if (newState == BottomSheetBehavior.STATE_EXPANDED) {
                mBottomSheetBehavior.setPeekHeight(getResources().getDimensionPixelOffset(R.dimen.filter_full));
            }*/
        }
    ...
    public void setupDialog(Dialog dialog, int style) {
        ...
        @LayoutRes int layout_resource = getArguments().getInt(INTENT_EXTRA_LAYOUT_RES, 0) == 0x0 ? LAYOUT_RES : getArguments().getInt(INTENT_EXTRA_LAYOUT_RES);
        View contentView = View.inflate(getContext(), layout_resource, null);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            contentView.setBackground(new ColorDrawable(Color.TRANSPARENT));
        }
        //getResources().getDimensionPixelOffset(R.dimen.filter_start)
        dialog.setContentView(contentView);
        bind(contentView);
        CoordinatorLayout.LayoutParams params = (CoordinatorLayout.LayoutParams) ((View) contentView.getParent()).getLayoutParams();
        CoordinatorLayout.Behavior behavior = params.getBehavior();
        // behavior.onMeasureChild();
        if (behavior != null && behavior instanceof BottomSheetBehavior) {
            mBottomSheetBehavior = (BottomSheetBehavior) behavior;
            mBottomSheetBehavior.setPeekHeight(getArguments().getInt(INTENT_EXTRA_SHEETPEEK_HEIGHT, 0));
    ...
    private void bind(View view) {
        ...
        mSpinner = (Spinner) view.findViewById(R.id.finder);
        mSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                //String mt = list_folders[position];
                if (listfolderstr != null && position - 1 > 0) {
                    Folder m = listfolderstr.get(position - 1);
                    Pickrx.get().post(EVENT_FOLDER_SYSTEM_DETECTION, m.getImages());
                }
            }
        ...
        okButtonKey = (TextView) view.findViewById(R.id.okbutton);
        ...
        imageAdapter = new ImagePickerAdapter(getContext(), images, selectedImages, this);
        ...
        okButtonKey.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void setDoneButtonCheck() {
        if (selectedImages.size() > 0) {
            if (okButtonKey != null) okButtonKey.setVisibility(View.VISIBLE);
        } else {
            if (okButtonKey != null) okButtonKey.setVisibility(View.GONE);
        }
    }
    ...
    private void setDetection() {
        mDetection = new ImageFolderDetection(getActivity(), new ImageFolderDetection.FolderGet() {
            @Override
    ...
    private void setupSpinner(List<Folder> list) {
        List<String> liststr = new ArrayList<>();
        liststr.add(getString(android.R.string.search_go));
        for (Folder ff : list) {
            liststr.add(ff.getFolderName());
        }
        listfolderstr = list;
        String[] strinist = liststr.toArray(new String[liststr.size()]);
        ArrayAdapter<String> spinArry = new ArrayAdapter<String>(getContext(), android.R.layout.simple_spinner_item, strinist);
        spinArry.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        mSpinner.setAdapter(spinArry);
